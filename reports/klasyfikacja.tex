...
W procesie kroswalidacji model uzyskał średni wynik \textbf{F1-macro} na poziomie $0{,}5268 \pm 0{,}0812$. Szczegółowe wyniki klasyfikacji na zbiorze testowym przedstawiono w Tabeli \ref{tab:dt_baseline_results}.
...
Model ten uzyskał najwyższy dotychczasowy średni wynik \textbf{F1-macro} w kroswalidacji wynoszący $0{,}6233 \pm 0{,}1004$. Szczegółowe metryki dla zbioru testowego zaprezentowano w Tabeli \ref{tab:dt_opt_results}.
...
Model po raz pierwszy poprawnie zidentyfikował obiekt z mniejszościowej klasy 2 (\textit{Recall} = 0,33), utrzymując przy tym bardzo wysoką precyzję (1,00). Ogólna dokładność (\textit{Accuracy}) wzrosła z $0{,}72$ do $0{,}81$, a miara \textbf{F1-macro} na zbiorze testowym osiągnęła poziom $0{,}71$, co stanowi istotny progres względem modelu bazowego ($0{,}60$) oraz modelu SMOTE bez optymalizacji ($0{,}49$).
...
Niska precyzja dla tej klasy ($0,33$) przy jednoczesnym wzroście ogólnego \textbf{F1-macro do 0,72} sugeruje, że model stał się „liberalny” w diagnozowaniu klasy mniejszościowej (częstsze fałszywe alarmy). Bardzo płytkie drzewo (\texttt{max\_depth: 3}) zapewnia doskonałą interpretowalność i odporność na przeuczenie.
...
Celem tego eksperymentu było przetestowanie bazowego modelu Lasu Losowego, składającego się ze 100 drzew decyzyjnych. W procesie kroswalidacji model uzyskał średni wynik \textbf{F1-macro} na poziomie $0{,}5602 \pm 0{,}0869$. Szczegółowa analiza metryk wykazuje istotne problemy z rozpoznawaniem klas mniejszościowych.
...
W procesie kroswalidacji model uzyskał średni wynik \textbf{F1-macro} na poziomie $0{,}6594 \pm 0{,}1142$, co stanowi istotną poprawę względem wersji bazowej ($0{,}5602$). Szczegółowe metryki uzyskane na zbiorze testowym zaprezentowano w Tabeli \ref{tab:rf_smote_results}.
...
Zoptymalizowany model uzyskał najwyższy dotychczas średni wynik \textbf{F1-macro} w kroswalidacji wynoszący $0{,}6774 \pm 0{,}0999$ (poprawa względem $0{,}6594$ dla modelu SMOTE bez optymalizacji). Szczegółowe metryki dla zbioru testowego zaprezentowano w Tabeli \ref{tab:rf_opt_results}.
...
Średni wynik \textbf{F1-macro} w kroswalidacji wyniósł $0{,}6752 \pm 0{,}0954$. Szczegółowe wyniki na zbiorze testowym przedstawiono w Tabeli \ref{tab:rf_random_results}.
...
W procesie kroswalidacji model uzyskał średni wynik \textbf{F1-macro} na poziomie $0{,}5003 \pm 0{,}0673$. 
...
W procesie kroswalidacji średni wynik \textbf{F1-macro} modelu wzrósł do $0{,}5537 \pm 0{,}0861$.
...
Średni wynik kroswalidacji uległ poprawie, osiągając \textbf{F1-macro} równe $0{,}5773 \pm 0{,}0699$.
